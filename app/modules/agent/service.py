...
class AgentService:
    ...
    async def ensure_builtin_agents(self) -> None:
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            await uow.commit()

    async def list_agents(
        self, user: CurrentUser, page: int = 1, page_size: int = 20
    ) -> PageResponse[AgentListItem]:
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            total = await uow.agents.count_definitions_for_user(user.id)
            pages = (total + page_size - 1) // page_size if total else 0
            offset = (page - 1) * page_size
            agents = await uow.agents.list_definitions_for_user(
    ...
    async def stream_chat(
        ...
        start = await self._start_stream_chat(
            user, agent_id, normalized_message, conversation_id, normalized_fund_code
        )
        ...
        try:
            ...
            await self._finish_run_failure(
                start.run_id, str(exc), _duration_ms(started_at)
            )
            yield error_event(str(exc))
            return

        assistant_content = "".join(assistant_chunks)
        await self._finish_run_success(
            run_id=start.run_id,
    ...
    async def list_reports(
        ...
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            total = await uow.agents.count_reports_for_user(
                user.id, agent_id, target_code
            )
            pages = (total + page_size - 1) // page_size if total else 0
            offset = (page - 1) * page_size
            rows = await uow.agents.list_reports_for_user(
                user.id, agent_id, target_code, offset=offset, limit=page_size
            )
            await uow.commit()
            return PageResponse(
                page=page,
    ...
    async def list_conversations(
        ...
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            await _get_enabled_agent_by_id(uow, user, agent_id)
            total = await uow.agents.count_conversations_for_user_agent(
                user.id, agent_id
            )
            pages = (total + page_size - 1) // page_size if total else 0
            offset = (page - 1) * page_size
            conversations = await uow.agents.list_conversations_for_user_agent(
                user.id, agent_id, offset=offset, limit=page_size
            )
            await uow.commit()
            return PageResponse(
                page=page,
    ...
    async def get_conversation_detail(
        self, user: CurrentUser, conversation_id: int
    ) -> AgentConversationDetailResponse:
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            conversation = await uow.agents.get_conversation_for_user(
                conversation_id, user.id
            )
            if conversation is None:
                raise NotFoundError("conversation not found")
            await _get_enabled_agent_by_id(uow, user, conversation.agent_id)
            messages = await uow.agents.list_visible_messages(conversation.id)
            await uow.commit()
    ...
    async def get_report_detail(
        ...
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            row = await uow.agents.get_report_for_user(report_id, user.id)
            if row is None:
                raise NotFoundError("report not found")
            report, agent = row
            await uow.commit()
            return AgentReportDetailResponse(
                **_to_report_item(report, agent).model_dump(), content=report.content
            )
    ...
    async def _start_stream_chat(
        ...
        async with self._uow_factory() as uow:
            await _ensure_builtin_agents(uow)
            agent = await _get_enabled_agent_by_id(uow, user, agent_id)
            conversation = await _get_or_create_conversation(
                uow, user, agent, conversation_id, message, fund_code
            )
            history = await _conversation_history(uow, conversation.id)
            uow.agents.add(
                AgentMessage(
            ...
            uow.agents.add(run)
            await uow.commit()
            await uow.agents.refresh(conversation)
            await uow.agents.refresh(run)
            return StreamStart(
        ...
        async with self._uow_factory() as uow:
            uow.agents.add(
                AgentMessage(
                    ...
                    content=str(
                        data.get("summary") or data.get("tool_name") or message_type
                    ),
    ...
    async def _finish_run_success(
        ...
        async with self._uow_factory() as uow:
            run = await uow.agents.get_run(run_id)
            conversation = await uow.agents.get_conversation(conversation_id)
            if output_text:
                uow.agents.add(
                    AgentMessage(
                        conversation_id=conversation_id,
...
async def _ensure_builtin_agents(uow) -> None:
    for item in BUILTIN_AGENTS:
        agent = await uow.agents.get_definition_by_code(item["code"])
        if agent is None:
            uow.agents.add(
                AgentDefinition(
                    **item,
    ...
    await uow.agents.flush()


async def _get_enabled_agent_by_id(
    uow, user: CurrentUser, agent_id: int
) -> AgentDefinition:
    agent = await uow.agents.get_enabled_definition_for_user(agent_id, user.id)
    if agent is None:
...
async def _get_or_create_conversation(
    ...
    if conversation_id:
        conversation = await uow.agents.get_conversation_for_user_agent(
            conversation_id, user.id, agent.id
        )
        if conversation is None:
            raise NotFoundError("conversation not found")
...
async def _conversation_history(uow, conversation_id: int) -> list[dict[str, str]]:
    messages = await uow.agents.list_history_messages(conversation_id)
...
def _to_conversation_message_item(
...
def _service_from_session(db: AsyncSession) -> AgentService:
    return AgentService(lambda: AgentUnitOfWork(lambda: db))


async def ensure_builtin_agents(db: AsyncSession) -> None:
    await _service_from_session(db).ensure_builtin_agents()


async def list_agents(
    db: AsyncSession, user: User | CurrentUser, page: int = 1, page_size: int = 20
) -> PageResponse[AgentListItem]:
    return await _service_from_session(db).list_agents(
        _to_current_user(user), page=page, page_size=page_size
    )
...
async def stream_chat(
    ...
    async for event in _service_from_session(db).stream_chat(
...
async def list_conversations(
    ...
    return await _service_from_session(db).list_conversations(
        _to_current_user(user), agent_id=agent_id, page=page, page_size=page_size
    )


async def get_conversation_detail(
    db: AsyncSession, user: User | CurrentUser, conversation_id: int
) -> AgentConversationDetailResponse:
    return await _service_from_session(db).get_conversation_detail(
        _to_current_user(user), conversation_id
    )
