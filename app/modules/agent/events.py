...
AgentEventType = Literal[
    "conversation", "step", "tool_call", "tool_result", "message", "error"
]
...
def step_event(
    step_id: str, title: str, status: str, error_message: str | None = None
) -> AgentStreamEvent:
...
def tool_call_event(
    tool_call_id: str, step_id: str, tool_name: str, input_data: dict[str, Any]
) -> AgentStreamEvent:
...
def tool_result_event(
    tool_call_id: str, status: str, summary: str, data: Any | None = None
) -> AgentStreamEvent:
    payload: dict[str, Any] = {
        "type": "tool_result",
        "tool_call_id": tool_call_id,
...
def message_event(content: str) -> AgentStreamEvent:
    return AgentStreamEvent(
        event="message", data={"type": "assistant_delta", "content": content}
    )
