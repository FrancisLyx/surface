...
def stream_fund_deep_analysis(payload: dict[str, Any], user: User, db: Session):
...
def stream_aggressive_ajia_analysis(payload: dict[str, Any], user: User, db: Session):
    fund_code = str(payload.get("fund_code") or "").strip()
    if not fund_code:
        raise ValueError("fund_code is required")
...
def stream_agent_chat_response(payload: dict[str, Any], history: list[dict[str, str]], persona: str = "professional"):
    message = str(payload.get("message") or "").strip()
    if not message:
        raise ValueError("message is required")

    fund_code = str(payload.get("fund_code") or "").strip()
    yield from _run_tool_calling_fund_agent(fund_code=fund_code, persona=persona, question=message, history=history)


def stream_favorite_fund_scan(payload: dict[str, Any], user: User, db: Session):
    page_size = int(payload.get("page_size") or 20)
    page_size = min(max(page_size, 1), 50)

    state = _favorite_scan_graph().invoke({"payload": {"page_size": page_size}, "user": user, "db": db})
...
def _run_tool_calling_fund_agent(
    ...
    messages = [SystemMessage(content=_fund_agent_system_prompt(persona))]
    ...
    messages.append(HumanMessage(content=_fund_agent_user_prompt(fund_code, persona, question=question)))
    ...
    yield from _stream_final_response(
        ...
        [
            *messages,
            HumanMessage(content="请基于已经获取的工具结果，直接输出最终 Markdown 报告。"),
        ],
    )
...
def _build_fund_agent_tools():
    ...
    def get_fund_value(fund_code: str) -> dict[str, Any]:
        ...
        result = fund_service.get_fund_value(FundValueRequest(fund_code=fund_code, source="auto"))
    ...
    def get_fund_profile(fund_code: str) -> dict[str, Any]:
        ...
        result = fund_service.get_fund_profile(FundProfileRequest(symbol=fund_code, year=str(date.today().year)))
    ...
    def get_fund_nav_trend_summary(fund_code: str) -> dict[str, Any]:
        ...
        return fund_service.get_fund_nav_trend_summary(fund_code)
...
def _fund_agent_user_prompt(fund_code: str, persona: str = "professional", question: str | None = None) -> str:
    current_question = question or f"请分析基金 {fund_code}。"
    fund_hint = f"当前基金代码：{fund_code}。" if fund_code else "当前没有指定基金代码；如果用户问题需要基金数据，先要求用户提供基金代码，或者基于已有上下文回答。"
    if persona == "aggressive_ajia":
        return f"""
请用“股神阿佳”的风格回答用户问题。
...
def _load_fund_data(state: FundAgentState) -> FundAgentState:
    fund_code = str(state["payload"]["fund_code"])
...
def _load_favorite_data(state: FundAgentState) -> FundAgentState:
    return {
        **state,
        "data": fund_tools.load_favorite_scan_data(state["db"], state["user"], page_size=int(state["payload"]["page_size"])),
    }
...
【自选基金扫描数据】
{_to_json(state["data"]["favorite_report"])}
